...
def test_staged_split_preserves_packets_without_per_file_atomicity(tmp_path: Path) -> None:
    """Dataset staging mode must preserve media while writing final paths."""

    first = tmp_path / "first.mp4"
    second = tmp_path / "second.mp4"
    combined = tmp_path / "combined.mp4"
    split_first = tmp_path / "staging" / "split-first.mp4"
    split_second = tmp_path / "staging" / "split-second.mp4"
    _make_video(first, 10)
    _make_video(second, 30)
    first_duration = video_duration(first)
    second_duration = video_duration(second)
    ...
    split_video(
        combined,
        [(slices[0], split_first), (slices[1], split_second)],
        atomic_output=False,
    )
